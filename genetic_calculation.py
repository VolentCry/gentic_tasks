...
def counter_of_repeated_gamets(list_of_one_parent_gamets: list) -> list[int, int]:
    """
...
def f2_universal_found(gamets_1: list, gamets_2: list, len_of_gamet: int) -> list[list, list]:
    """
    ...
    if counter_p1 == counter_p2 and counter_p1 != 0 and counter_p2 != 0:
        gamets = list(combinations(all_gamets, r = len_of_pair1))
    elif counter_p1 == 0 and counter_p1 == 0:
        gamets = list(combinations(all_gamets, r = len_of_gamet))
    ...
    return finally_f2, new_gamets


def pairs_found(f1: str, length_of_pair: int) -> list:
    ...
    for combos in product(f1, repeat=length_of_pair):
...
def universal_f2_found(f1: str, length_of_pair: int) -> list:
...
def genetic_programme(parent_one: str, parent_two: str, mendel_table_status: int):
    ...
    try:
        ...
        for i in parent_one:
            if i not in eng_alphabet:
                raise('Error: родители должны быть записаны латинскими буквами')
        for i in parent_two:
            if i not in eng_alphabet:
                raise('Error: родители должны быть записаны латинскими буквами')
        ...
        S = list(set((gamet_parent_one.replace(' ', '').replace(',', '')) + (gamet_parent_two.replace(' ', '').replace(',', ''))))

        if len(gamet_parent_one) == len(gamet_parent_two): # проверяет, одинаковое ли количество гамет у разных родителей
            gibrids_one = ''.join(S)
            num = 0
            ...
            if parent_one == parent_two:
                ...
                if len(parent_one) == 2 and len(parent_two) == 2:
                    ...
                    mendel_table = table_maker(f1, len(gamet_parent_one.split(', ')), f2)
                f3 = gamet_parent_one # Гаметы первого родителя
                f4 = gamet_parent_two # Гаметы второго родителя

                if mendel_table_status == 1:
                    return f1, f2, f3, f4, mendel_table
                elif mendel_table_status == 0:
                    return f1, f2, f3, f4
                else:
                    raise("Неверное значение аргумента mendel_table_status")
...
if __name__ == '__main__':
    # Тесты
    print(f"1. {genetic_programme('AaBb', 'AaBb', 0)}") # f2, g1, g2, pairs
    print(f"2. {genetic_programme('AABB', 'aabb', 1)}") # f1, f2, g1, g2
    print(f"3. {genetic_programme('aaBB', 'AAbb', 0)}") # f1, f2, g1, g2, pairs
    print(f"4. {genetic_programme('Aa', 'Aa', 0)}") # f2, g1, g2
    print(f"5. {genetic_programme('AA', 'aa', 0)}") # f1, f2, g1, g2
    print(f"6. {genetic_programme('Aa', 'aa', 0)}") # f1, g1, g2
    print(f"7. {genetic_programme('Aa', 'AA', 0)}") # f1, g1, g2
    print(f"8. {genetic_programme('AABBCC', 'aabbcc', 0)}") # f1, f2, g1, g2
    print(f"9. {genetic_programme('AABBcc', 'aabbCC', 0)}") # f1, f2, g1, g2, pairs
    print(f"10. {genetic_programme('AaBbCc', 'AaBbCc', 0)}") # f2, g1, g2, pairs
    print(f"11. {genetic_programme('AABBCCDDEE', 'aabbccddee', 0)}") # f1, f2, g1, g2, pairs